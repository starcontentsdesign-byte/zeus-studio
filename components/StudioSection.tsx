...
export default function StudioSection({
  ...
  useEffect(() => {
    if (authLoading || postsLoading) return;
    if (!isAdmin || !user?.id) return;
    if (studioPosts.length > 0) return;
    ...
    void (async () => {
      try {
        const response = await fetch('/api/admin/studio-posts/bootstrap', {
          method: 'POST'
        });
        ...
        if (!response.ok) {
          throw new Error(
            payload?.message || '기본 Studio 게시물 부트스트랩에 실패했습니다.'
          );
        }

        if (typeof payload?.inserted === 'number' && payload.inserted > 0) {
          toast({
            title: '기본 Studio 게시물 생성 완료',
            description: `${payload.inserted}개의 기존 이미지를 게시물로 등록했습니다.`
          });
        }
